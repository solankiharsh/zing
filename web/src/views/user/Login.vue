      ...
      this.codeLoginForm.validateFields(async (err, values) => {
        ...
        try {
          ...
          if (res.code === 1 && res.data?.token) {
            // Save token (save to storage first so the request interceptor can read it)
            const expiresAt = new Date().getTime() + 7 * 24 * 60 * 60 * 1000
            storage.set(ACCESS_TOKEN, res.data.token, expiresAt)
            this.$store.commit('SET_TOKEN', res.data.token)

            // Save user info (from the login API response userinfo)
            if (res.data.userinfo) {
              const userInfoData = { ...res.data.userinfo }
              // Ensure is_demo field exists to prevent GetInfo from treating cache as expired
              if (typeof userInfoData.is_demo === 'undefined') {
                userInfoData.is_demo = false
              }

              // Save to storage so GetInfo can read it
              ...
              if (userInfoData.role) {
                // Handle role being an object or an array
                ...
                // If no role info, set a default role object to prevent the route guard from blocking
            ...
            // Ensure roles have been set correctly (use Vue.nextTick to confirm state update)
            await this.$nextTick()

            // Verify token and roles are correctly set
            ...
            // Wait briefly to ensure the token is set in the request interceptor
            await new Promise(resolve => setTimeout(resolve, 200))

            // Reset routes, force regeneration (based on the new user's roles)
            // Note: ResetRoutes only clears routes, it does not trigger the route guard
            this.$store.dispatch('ResetRoutes')

            // Navigate directly; the route guard will check roles — if roles are set, it won't call GetInfo
            ...
            this.$router.push({ path: '/' }).then(() => {
              ...
              // Even if navigation fails, show success message
              this.$notification.success({
                message: isNew ? (this.$t('user.login.welcomeNew') || 'Welcome!') : 'Welcome',
                description: isNew
                  ? (this.$t('user.login.accountCreated') || 'Your account has been created.')
        ...
        try {
          ...
          if (res.code === 1) {
            ...
            if (res.data?.token) {
              ...
              // Save user info (from the registration API response userinfo)
              if (res.data.userinfo) {
                const userInfoData = { ...res.data.userinfo }
                // Ensure is_demo field exists to prevent GetInfo from treating cache as expired
                if (typeof userInfoData.is_demo === 'undefined') {
              ...
              this.$router.push({ path: '/' }).then(() => {
                this.$notification.success({
                  message: 'Welcome',
                  description: `${timeFix()}, welcome to MarketLabs!`
                })
              }).catch(err => {
                console.error('Router push error:', err)
                // Even if navigation fails, show success message
...
.main {
  ...
  /* Intro: sits just above login dialog */
  .auth-intro {
    text-align: center;
    margin-bottom: 12px;
    max-width: 420px;
  ...
  /* Card: subtle elevation on grid background, clear separation */
  .auth-card {
    min-width: 360px;
    width: 420px;
