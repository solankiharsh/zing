<template>
  <div class="portfolio-container" :class="{ 'theme-dark': isDarkTheme, embedded: embedded }">
    <!-- Asset overview - Row 1: core metrics -->
    ...
    <!-- Asset overview - Row 2: detailed statistics -->
      ...
      <div class="positions-section">
        <div class="section-header">
          ...
          <div class="header-actions">
            ...
            <!-- Group filter (only shown in grid view) -->
            <a-select
              v-if="viewMode === 'grid'"
      ...
      <a-form :form="positionForm" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        ...
        <a-form-item :label="$t('portfolio.form.symbol')">
          ...
          >
            ...
            <!-- Manual input option: shown when search yields no results and there is input -->
            <a-select-option
    ...
    >
      ...
      <a-form :form="alertForm" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        ...
        <!-- Notification channels - bound via v-model -->
...
export default {
  ...
  data () {
    return {
      ...
      searchTimer: null,
      selectedSymbolName: '',
      symbolSearchKeyword: '', // Current search keyword for manual input
      // Price refresh
      priceRefreshTimer: null,
      lastSyncTime: null, // Last sync time
      isSyncing: false, // Whether currently syncing
      // Groups
      groups: [],
      selectedGroup: '',
      // View mode
      viewMode: 'grid', // 'grid' or 'group'
      activeGroups: [], // Expanded groups in collapse panel
      // Alerts
      alerts: [],
      loadingAlerts: false,
  ...
  computed: {
    ...
    // Profit/loss position statistics
    profitLossStats () {
  ...
  methods: {
    ...
    // Refresh prices (force refresh, skip cache)
    async refreshPrices () {
    ...
    // Calculate total P&L for a group
    ...
    handleSymbolSearch (value) {
      // Save search keyword for manual input feature
      ...
      this.searchTimer = setTimeout(async () => {
        ...
        try {
          ...
          if (res && res.code === 1) {
            ...
            // No search results, clear the list but keep the keyword for manual input
            this.symbolSearchResults = []
          }
        } catch (e) {
          // Search failed, still allow manual input
          this.symbolSearchResults = []
        }
      }, 300)
    },
    handleSymbolSelect (value, option) {
      // First look up from search results
      const item = this.symbolSearchResults.find(s => s.symbol === value)
      if (item) {
        this.selectedSymbolName = item.name
      } else {
        // Manual input case, leave name empty — backend will try to fetch it
    ...
    showAddAlertForPosition (pos) {
      // Check if an alert already exists for this position; if so, edit it
      ...
      // Create a new alert - use user default notification settings
    ...
    async handleSaveAlert () {
      this.alertForm.validateFields(async (err, values) => {
        ...
        try {
          // Build notification targets - use values configured in user profile
          const targets = {}
          ...
          const data = {
            ...
            notification_config: {
              // Use values bound via v-model
              channels: this.alertChannels.length > 0 ? this.alertChannels : ['browser'],
              targets: targets,
              language: this.$store.getters.lang || 'en-US' // Save current language
            },
    ...
    async runMonitorNow (id) {
      ...
      try {
        // Pass current language to backend, use async mode
        const currentLang = this.$store.getters.lang || 'en-US'
        const res = await runMonitor(id, { language: currentLang, async: true })
        if (res && res.code === 1) {
          // Async mode: backend returns immediately, executes in background
          if (res.data?.status === 'running') {
            ...
            // Sync mode returns result (backward compatible)
    ...
    formatTime (timestamp) {
      if (!timestamp) return '-'
      let d
      // If it's a number (seconds timestamp), multiply by 1000
      if (typeof timestamp === 'number') {
        d = new Date(timestamp * 1000)
      } else if (typeof timestamp === 'string') {
        // If it's a pure numeric string (seconds timestamp)
        if (/^\d+$/.test(timestamp)) {
...
.portfolio-container {
  ...
  &.theme-dark {
    ...
    .summary-card {
      ...
      // Dark mode total market value card
      &.total-value {
...
.summary-card {
  ...
  // Total market value card - vivid gradient style
  &.total-value {
...
@media screen and (max-width: 768px) {
  ...
  .summary-card {
    ...
    // Total market value card spans full width on mobile
    &.total-value {
      grid-column: span 2;
      padding: 16px;
...
@media screen and (max-width: 480px) {
  ...
  .summary-section {
    ...
    &.secondary {
      // Second row scrolls horizontally on extra small screens
      display: flex;
      overflow-x: auto;
      padding-bottom: 8px;
