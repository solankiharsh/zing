<template>
  ...
  >
    <!-- Preview image / Default generated cover -->
    <div class="card-cover" :style="coverStyle">
      ...
      />
      <!-- Default cover: gradient background + title -->
...
export default {
  ...
  computed: {
    ...
    // Generate a fixed gradient color based on indicator ID
    coverGradient () {
      const index = (this.indicator.id || 0) % GRADIENT_PRESETS.length
      return GRADIENT_PRESETS[index]
    },
    // Generate indicator name initials
    indicatorInitials () {
      const name = this.indicator.name || 'I'
      // If Chinese, take the first two characters
      if (/[\u4e00-\u9fa5]/.test(name)) {
        return name.slice(0, 2)
      }
      // If English, take the uppercase initials
