  ...
  methods: {
    async loadDecisions () {
      ...
      try {
        ...
        if (res.code === 1) {
          // Process the returned data format
          const data = res.data
          let decisions = []
          let total = 0
          ...
          if (data && typeof data === 'object') {
            if (Array.isArray(data)) {
              ...
              // New format: contains decisions and total
...
.ai-decision-records {
  ...
  // Ensure hold tag (cyan) has sufficient contrast in all themes
  /deep/ .ant-tag {
    &.ant-tag-cyan {
...
<style lang="less">
// Global styles: tag optimization for dark theme
.trading-assistant.theme-dark {
  .ai-decision-records {
    ...
    /deep/ .ant-tag {
      // Hold tag uses cyan, ensure visibility on dark background
      &.ant-tag-cyan {
        background-color: #13c2c2 !important;
        border-color: #13c2c2 !important;
        color: #fff !important;
      ...
      // Other tags optimization for dark theme
      &.ant-tag-green {
      ...
      // Default tag optimization for dark theme
      &.ant-tag-default {
    ...
    // Progress bar uses theme color in dark theme
    /deep/ .ant-progress {
      .ant-progress-bg {
        background-color: var(--primary-color, #1890ff);
      }
