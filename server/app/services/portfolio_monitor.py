...
def _get_alert_message(alert_type: str, **kwargs) -> str:
    """Get alert message for the given type."""
    template = ALERT_MESSAGES.get(alert_type, '')
    if template:
...
def _get_alert_title() -> str:
    """Get alert title."""
    return ALERT_MESSAGES.get('alert_title', 'Alert')
...
def _build_html_report(
    ...
    texts = {
        ...
        'subtitle': 'Generated by MarketLabs Fast AI Analysis Engine',
...
def _build_telegram_report(
    ...
    lines.extend([
        ...
        "<i>Generated by MarketLabs Multi-Agent System</i>"
    ])
...
def _send_monitor_notification(
    ...
    try:
        ...
        if not result.get('success'):
            error_title = f"⚠️ Monitor Failed: {monitor_name}"
...
def run_single_monitor(monitor_id: int, override_language: str = None, user_id: int = None) -> Dict[str, Any]:
    ...
    Args:
        ...
        override_language: Optional language override (e.g., 'en-US')
...
def notify_strategy_signal_for_positions(market: str, symbol: str, signal_type: str, signal_detail: str, user_id: int = None):
    ...
    try:
        ...
        for pos in positions:
            ...
            message = f"""Strategy issued a {signal_type} signal!
    ...
    while not _stop_event.is_set():
        try:
            ...
            with get_db_connection() as db:
                ...
                cur.execute(
                    """
                    SELECT id, user_id FROM ml_position_monitors
