...
class SignalNotifier:
    ...
    Notification config:
    - Users set telegram_bot_token, telegram_chat_id, email etc. in profile
    - When creating strategy/monitor, system uses user's notification targets

    Shared config (admin in system settings):
    - SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASSWORD, SMTP_FROM, SMTP_USE_TLS (email, shared)
    - TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_FROM_NUMBER (SMS, shared)
...
"""
    ...
    def _notify_webhook(
        ...
        *,
        ...
        - Signing: notification_config.targets.webhook_signing_secret
        ...
        headers: Dict[str, str] = {
            ...
            "User-Agent": "MarketLabs/1.0 (+https://www.marketlabs.com)",
        }
    ...
    def _notify_telegram(
        ...
        *,
        ...
        # User must set telegram_bot_token in profile
        token = (token_override or "").strip()
        if not token:
            return False, "missing_telegram_bot_token (configure Telegram Bot Token in profile)"
        if not chat_id:
